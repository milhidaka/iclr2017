...
batch division for small gpu ram
...
assert args.batchsize % args.div == 0
...
serializers.save_npz(args.out % optimizer.t, model)
serializers.save_npz(args.outstate % optimizer.t, optimizer)
...
def read_image(path, center=False, flip=False):
    ...
    #image /= 255#not necessary for vgg
...
def log_result():
    ...
    while True:
        ...
        if train:
            ...
            if val_count == len(val_list):
                mean_loss = val_loss * args.val_batchsize / len(val_list)
                mean_error = 1 - val_accuracy * args.val_batchsize / len(val_list)
                print(file=sys.stderr)
...
def train_loop():
    ...
    while True:
        ...
        if model.train:
            sample_len = len(inp[0])
            once_len = sample_len / args.div
            model.cleargrads()
            for div_i in six.moves.range(args.div):
                x = chainer.Variable(xp.asarray(inp[0][div_i*once_len:(div_i+1)*once_len]), volatile=volatile)
                t = chainer.Variable(xp.asarray(inp[1][div_i*once_len:(div_i+1)*once_len]), volatile=volatile)
                div_loss = model(x, t)
                offset_loss = div_loss * (1.0 / args.div)#to make gradient 1.0/args.div
